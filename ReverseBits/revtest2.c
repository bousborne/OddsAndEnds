...
void ReverseBits(unsigned char *arr, int len_arr)
{
  //Reverse the bits in each individual character
  for (int j = 0; j < len_arr; j++)
  {
    // flip first and last 4 bits
    // 12345678 -> 56781234
    arr[j] = (arr[j] & 0xF0) >> 4 | (arr[j] & 0x0F) << 4;
    // flip consecutive groups of 2 bits
    // 56781234 -> 78563412
    arr[j] = (arr[j] & 0xCC) >> 2 | (arr[j] & 0x33) << 2;
    //flip consecutive groups of 1 bit
    //78563412 -> 87654321
    arr[j] = (arr[j] & 0xAA) >> 1 | (arr[j] & 0x55) << 1;
  }
