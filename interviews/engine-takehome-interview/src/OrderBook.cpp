...
template <typename T> bool OrderBook::isBuy(T x) {
    return x->sSide == "BUY";
}
...
bool OrderBook::sellSortCompare(const std::shared_ptr<Order> &order1,
...
bool OrderBook::buySortCompare(const std::shared_ptr<Order> &order1,
                               const std::shared_ptr<Order> &order2) {
    if (order1->price != order2->price) {
        return (order1->price > order2->price);
    }
...
bool OrderBook::timeSortCompare(const std::shared_ptr<Order> &order1,
                                const std::shared_ptr<Order> &order2) {
    return (order1->orderTime < order2->orderTime);
}
...
void OrderBook::match(std::shared_ptr<Order> &order) {
    if (isBuy(order)) {
        ...
        for (it = sellList.begin(); it != sellList.end(); it++) {
            if ((order->sInstrument == (*it)->sInstrument) &&
                (order->price >= (*it)->price) && ((*it)->quantity != 0)) {
                makeTrade(order, (*it), order->price,
                          MIN(order->quantity, (*it)->quantity));
...
void OrderBook::makeTrade(std::shared_ptr<Order> &buyerOrder,
                          std::shared_ptr<Order> &sellerOrder, int price,
                          int quantity) {
    std::cout << "TRADE " << buyerOrder->sInstrument << " "
              << sellerOrder->orderId << " " << buyerOrder->orderId << " "
              << quantity << " " << price << std::endl;
    std::shared_ptr<Trade> trade(
        new Trade("TRADE", buyerOrder->sInstrument, buyerOrder->orderId,
                  sellerOrder->orderId, quantity, price));
...
void OrderBook::printRemaining() {
    // These sorts are unnecessary for matching, but rather to just make the end
    // result match the sort of the example
    ...
    for (const auto &s : sellList) {
        if (s->quantity != 0) {
            std::cout << s->orderId << " " << s->sSide << " " << s->sInstrument
                      << " " << s->quantity << " " << s->price << std::endl;
        }
    }
    for (const auto &b : buyList) {
        if (b->quantity != 0) {
            std::cout << b->orderId << " " << b->sSide << " " << b->sInstrument
                      << " " << b->quantity << " " << b->price << std::endl;
        }
    }
}
...
std::ostream &operator<<(std::ostream &os, const OrderBook &book) {
    ...
    for (const auto &s : book.sellList) {
        if (s->quantity != 0) {
            os << s->orderId << " " << s->sSide << " " << s->sInstrument << " "
               << s->quantity << " " << s->price << std::endl;
        }
    }
    for (const auto &b : book.buyList) {
        if (b->quantity != 0) {
            os << b->orderId << " " << b->sSide << " " << b->sInstrument << " "
               << b->quantity << " " << b->price << std::endl;
        }
    }
    return os;
